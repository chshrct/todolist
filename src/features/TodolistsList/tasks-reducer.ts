...
export const addTaskTC =
  ...
  (dispatch: Dispatch<AppRootActionType>) => {
    dispatch(setAppStatusAC("loading"));
    todolistsAPI
...
export const updateTaskTC =
  ...
  (dispatch: Dispatch<AppRootActionType>, getState: () => AppRootStateType) => {
    ...
    dispatch(setAppStatusAC("loading"));
    dispatch(setTaskEntityStatusAC(task.todoListId, task.id, "loading"));
    todolistsAPI
      ...
      .then((res) => {
        if (res.data.resultCode === 0) {
          ...
          dispatch(setAppStatusAC("succeeded"));
          dispatch(
            setTaskEntityStatusAC(task.todoListId, task.id, "succeeded")
          );
        } else {
          handleServerAppError(res.data, dispatch);
          dispatch(setTaskEntityStatusAC(task.todoListId, task.id, "failed"));
        }
      })
      .catch((error) => {
        handleServerNetworkError(error, dispatch);
...
export type SetTaskEntityStatusActionType = ReturnType<
  typeof setTaskEntityStatusAC
>;
